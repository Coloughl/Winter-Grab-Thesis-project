...
eems <- eem_read("staRdom_Practice/EEMs", import_function = "aqualog")


#read in metadata
metadata <- ("staRdom_Practice/metadata.table.csv")
meta <- read.table(metadata, header = TRUE, sep = ",", dec = ".")
...
absorbance_path = ("staRdom_Practice/ABS")
...
#Remove blanks
eems <- eem_extract(eems, c("nano", "miliq", "milliq", "mq", "blank","DI"),ignore_case = TRUE)
